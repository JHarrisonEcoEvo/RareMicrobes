...
load("./CNVRG_16S_conservative.RData")
...
transformed_out <- ps
for(i in 1:8){
  transformed_out$p[,i,] <-  ps$p[,i,] / ps$p[,i,which(names(t_newdat) == "ISD")-1]
}
entropy_p_shan <- list()
entropy_p_simps <- list()

for(i in 1:reps){
  entropy_p_shan[[i]] <- exp(vegan::diversity(transformed_out$p[,i,-contamHits], index = "shannon"))
  entropy_p_simps[[i]] <- exp(vegan::diversity(transformed_out$p[,i,-contamHits], index = "simpson"))
}
save(entropy_p_shan, entropy_p_simps,t_newdat, file = "entropy_p_bacteria_conservative.RData")
# 
# rm(list=ls())
# load("./CNVRG_16S.RData") 
# ps <- rstan::extract(modelOut, "p")
# reps <- rapply(ps, dim, how="list")$p[2]
# 
# transformed_out <- ps
...
# 
# for(i in 1:reps){
#   entropy_p_shan[[i]] <- exp(vegan::diversity(transformed_out$p[,i,], index = "shannon"))
#   entropy_p_simps[[i]] <- exp(vegan::diversity(transformed_out$p[,i,], index = "simpson"))
# }
# save(entropy_p_shan, entropy_p_simps, t_newdat, file = "entropy_p_bacteria.RData")

rm(list=ls())
load("./data/entropy_p_bacteria_conservative.RData")
shan <- unlist(lapply(entropy_p_shan, FUN = mean))
simps <- unlist(lapply(entropy_p_simps, FUN = mean))

#sanity check
#mean(entropy_p_shan[[130]]) # same as last element from preceeding call to lapply
...
#sanity check
#dat5$plant == t_newdat$metadat.plant
#duplicated(dat5$plant)

#Imputation of missing values for predictors of interest
#can be performed for each response variable
...
names(dat4) <-  c(
  ...
  "shannon",
  "simpson",
  "height", #the box measurements get combined below
  "width",
  "length",
  "sla_avg",
  "percN",
...
starts <-  bounder(dat4$treatment[-rmv])[1]
ends <- bounder(dat4$treatment[-rmv])[2]
out <- modelRun(x = dat3$simpson,
                datz = dat3,
...
dat5 <- dat4
test <- rfImpute(x = dat5[,5:11], y = dat5$simpson)
dat5[,5:11] <- test[,2:8]

starts <-  bounder(dat5$treatment)[1]
ends <- bounder(dat5$treatment)[2]

modelRun(x = dat5$simpson,
         datz = dat5,
         starts = unlist(starts), 
         ends = unlist(ends))
