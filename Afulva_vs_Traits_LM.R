...
afulva <- read.table("./data//levtaurMatchesITS", stringsAsFactors=F, header = F) 

afulvaHits <- which(names(dat) %in% afulva$V1)

afulva <- rowSums(dat[,afulvaHits])
...
#Impute data that are missing. 
dat5 <- dat4
test <- rfImpute(x = dat5[,4:11], y = dat5$afulva)
dat5[,4:11] <- test[,2:8]

starts <-  bounder(dat5$treatment)[1]
ends <- bounder(dat5$treatment)[2]

modelRun(x = dat5$afulva,
         datz = dat5,
