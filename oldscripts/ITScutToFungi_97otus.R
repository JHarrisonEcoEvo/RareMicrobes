...
tax=read.delim("data/ITS_sequenceProcessing/combinedTaxonomy97otus.txt", header=F)
otus=read.delim("data/ITS_sequenceProcessing/otuTable97otus.txt", header=T)
...
#phylum often match nonfungal taxa on NCBI...so I remove those too.
...
fungiOnly = fungiOnly[which(nchar(as.character(fungiOnly[,3])) > 7 | nchar(as.character(fungiOnly[,4])) > 7),]
...
write.csv(data.frame(newotus$sampleName, newotus[,2:38]),file="data/fungiOTUtable97otus.csv", row.names = F)
