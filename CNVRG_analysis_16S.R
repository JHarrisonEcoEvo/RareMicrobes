...
dat <- read.table("./data/otutable16S_gg_cpDNAmtDNAISDduds_sum", 
...
newnames <- gsub("X16S_\\w+_\\w+_Harrison_asle_(\\d+)asle", "\\1", names(dat))
...
#sanity check for what comes next
...
# 
# modelOut <- varInf(
#   countData = data.frame(metadat$plant, t_newdat+1 ),
#   starts = indexer(metadat$treament)$starts,
#   ends = indexer(metadat$treament)$ends,
#   algorithm = "meanfield",
#   output_samples = 1000,
#   params_to_save = c("pi", "p")
# )
diff_abund(model_output = modelOut, countData = data.frame(metadat$plant, t_newdat+1 ))

save.image(file = "CNVRG_16S_conservative.RData")
