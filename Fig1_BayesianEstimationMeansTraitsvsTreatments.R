...
coefvar <- function(x, y){
  cv <- sqrt(x) / y
  for(i in 2:4){
    print(paste("1 vs", i, sep = ""))
    print(table((cv[1,,] - cv[i,,]) > 0)[1] / 2000)
  }
  print("2 vs 3")
  print(table((cv[2,,] - cv[3,,]) > 0)[1] / 2000)
  print("2 vs 4")
  print(table((cv[2,,] - cv[4,,]) > 0)[1] / 2000)
  print("3 vs 4")
  print(table((cv[3,,] - cv[4,,]) > 0)[1] / 2000)
  return(list(means = apply(cv, 1, mean),
              quantiles = apply(cv, 1, quantile, probs=c(0.025, 0.975))))
  
}

out_mat <- matrix(nrow = 9, ncol = 5)
out_mat_var <- matrix(nrow = 9, ncol = 5) #for trait variance measurements
...
out_mat_var[1,1] <- "Size"

for(i in 1:4){
  out_mat_var[1,i+1] <- paste(round(unlist(cv$means[i]),4),
                           " (",
                           round(cv$quantiles[1,i], 4),
                           ",",
                           round(cv$quantiles[2,i], 4),
                           ")", 
                           sep="")
}
...
out_mat_var[2,1] <- "Num. of leaves"

for(i in 1:4){
  out_mat_var[2,i+1] <- paste(round(unlist(cv$means[i]),4),
                              " (",
                              round(cv$quantiles[1,i], 4),
                              ",",
                              round(cv$quantiles[2,i], 4),
...
for(i in 1:4){
  out_mat_var[3,i+1] <- paste(round(unlist(cv$means[i]),4),
                              " (",
                              round(cv$quantiles[1,i], 4),
                              ",",
                              round(cv$quantiles[2,i], 4),
...
out_mat_var[4,1] <- "Specific leaf area"

for(i in 1:4){
  out_mat_var[4,i+1] <- paste(round(unlist(cv$means[i]),4),
                              " (",
                              round(cv$quantiles[1,i], 4),
                              ",",
                              round(cv$quantiles[2,i], 4),
...
out_mat_var[5,1] <- "delta N15"

for(i in 1:4){
  out_mat_var[5,i+1] <- paste(round(unlist(cv$means[i]),4),
                              " (",
                              round(cv$quantiles[1,i], 4),
                              ",",
                              round(cv$quantiles[2,i], 4),
...
out_mat_var[6,1] <- "% swainsonine"

for(i in 1:4){
  out_mat_var[6,i+1] <- paste(round(unlist(cv$means[i]),4),
                              " (",
                              round(cv$quantiles[1,i], 4),
                              ",",
                              round(cv$quantiles[2,i], 4),
...
for(i in 1:4){
  out_mat_var[7,i+1] <- paste(round(unlist(cv$means[i]),4),
                              " (",
                              round(cv$quantiles[1,i], 4),
                              ",",
                              round(cv$quantiles[2,i], 4),
...
for(i in 1:4){
  out_mat_var[8,i+1] <- paste(round(unlist(cv$means[i]),4),
                              " (",
                              round(cv$quantiles[1,i], 4),
                              ",",
                              round(cv$quantiles[2,i], 4),
...
# dat_percC13 <- dat[-which(is.na(dat$d13C....vs..VPDB.)),]
# start <- indexer(dat_percC13$treament)[[1]]
# end <- indexer(dat_percC13$treament)[[2]]
# out <- modelRun(dat_percC13$d13C....vs..VPDB., start, end)
# c13out<- c("% C",  
#                  paste(round(unlist(out$means),2),
#                        " (",
#                        round(sapply(out$quantiles, "[[", 1), 2),
...
#                        ")", 
#                        sep=""))
# boxplot(dat_percC13$d13C....vs..VPDB. ~ dat_percC13$treament)
...
out_mat_var[9,1] <- "NDFA"

for(i in 1:4){
  out_mat_var[9,i+1] <- paste(round(unlist(cv$means[i]),4),
                              " (",
                              round(cv$quantiles[1,i], 4),
                              ",",
                              round(cv$quantiles[2,i], 4),
...
colz <- c("cyan2", "cyan4", "darkgoldenrod2", "darkgoldenrod4")
...
stripchart(log(dat$vol) ~ droplevels(dat$treament),
           vertical = TRUE, 
           data = dat, 
           method = "jitter", 
           pch = 20, 
...
boxplot(log(dat$vol) ~ droplevels(dat$treament), 
        na.omit = T,
        add=T,
        las=2,
        outline=F,
...
axis(2,las=2, 
     at = c(seq(2,12,by = 2)),
     labels = c(seq(2,12,by = 2))
)
title(ylab=expression(paste("Plant volume (ln ",cm^3,")",sep="")), cex.lab=1.5, line=3.2,xpd = NA)
...
axis(1,las=2, labels = c("","","",""), at = c(1,2,3,4))
axis(2,las=2, 
     at = c(seq(0.3,0.9,by = 0.1)),
     labels = c(seq(0.3,0.9,by = 0.1)))
...
axis(1,las=2, labels = c("","","",""), at = c(1,2,3,4))
axis(2,las=2,
     at = c(seq(0.08,0.2,by = 0.02)),
     labels = c(seq(0.08,0.2,by = 0.02)))
